...
3. 如果当前轮次不是最后一轮，nextQuestion 要基于项目和用户回答继续追问；
...
6. feedback 的严格程度要符合反馈方式；
7. nextQuestion 的深度要符合问题深度和重点追问方向，但仅在不是最后一轮时生成；
8. 不要编造候选人没有提到的事实，可以围绕已有信息继续追问。
9. 如果提供了项目上下文参考，只能作为辅助背景；不要照抄项目档案，不要编造项目档案中不存在的信息。
...
10. feedback 必须根据本轮项目问题和用户回答生成具体内容；nextQuestion 仅在不是最后一轮时生成具体内容；
11. JSON 必须符合以下格式：

{
  "feedback": "对用户本轮回答的具体评价",
  "nextQuestion": "不是最后一轮时填写下一轮追问；最后一轮时填写空字符串"
}
